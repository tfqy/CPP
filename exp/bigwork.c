...
struct code_addr_node
{ // 包含取件码地址的节点,是手机号广义表的子表元素
  code_pointer code_P;
          ...
          if (sign_in(DL, &dP) == TRUE) // 登录成功
          {
            ...
            while (1)
            {
              ...
              if (i == 1)
              {
                ...
                {
                case 'S':
                  put_stuff(cL, dP, lockers_S, pL, stuff, 1);
                  break; // 最后一个为大小
                case 'M':
                  put_stuff(cL, dP, lockers_M, pL, stuff, 2);
                  break;
                case 'L':
                  put_stuff(cL, dP, lockers_L, pL, stuff, 3);
                  break;
                default:
...
code_pointer locate_code_list(code_list head, char code[7])
{ // 传入取件码,链表,查找对应取件码的取件码节点地址
...
{ // 子链索引表查询(其查询到的节点的前驱结点存放在phonenum的sublist_prior)
...
{ // 取件pL为以电话分类的索引表
  ...
  while (1)
  {
    ...
    while (1)
    {
      ...
      if (cP->phonenum_p->sublist_head->next == NULL)
      {                                       // 表空,该号码所有快递均取走
